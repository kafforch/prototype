...
class PlanRepoTestsBase ( unittest.TestCase ):
    def setUp(self):
        ...
        self.plan_json3 = open ( 'tests/orch_tests/json/plan_03.json', 'r' ).read ( )

    def tearDown(self):
        plan_repo.purge_all_plans()


class PlanRepoTests ( PlanRepoTestsBase ):
    ...
    def test_get_not_started_timed_plan_ids(self):
        plan_repo.save_new_plan ( self.plan_json3 )
        plan_repo.save_new_plan ( self.plan_json3 )
        self.assertEqual(len(plan_repo.get_not_started_timed_plan_ids()), 2)
